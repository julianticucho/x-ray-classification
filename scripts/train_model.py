...
def train_model(
    ...
    dl_factory = DataLoaderFactory(labels_df, img_dir, num_workers=num_workers)
    model_factory = ModelFactory()

    train_df, _ = pp_factory.get(preprocessing_name)
    print(f"Training with {len(train_df)} samples")
    dataloader = dl_factory.get(dataloader_name)
    model = model_factory.get(model_name)
    model.train_model(dataloader, num_epochs=num_epochs, lr=lr)
...
if __name__ == '__main__':
    train_model(
        ...
        preprocessing_name='example', 
        dataloader_name='basic', 
        model_name='mlp', 
        output_path='results/models/example.pth',
        num_epochs=2,
        lr=0.001,
        num_workers=11,
    )
